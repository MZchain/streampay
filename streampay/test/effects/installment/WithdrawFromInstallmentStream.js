...
function shouldBehaveLikeInstallmentWithdrawStream(alice, bob, eve) {
        ...
        describe("when the caller is the sender of the stream", function() {
            ...
            it("reverts", async function() {
                await truffleAssert.reverts(
                    this.streamPayOfInstallment.withdrawInstallmentStream(this.streamId, FIVE_UNITS, opts),
    ...
    describe("when the stream does not exist", function() {
        ...
        it("reverts", async function() {
            ...
            await truffleAssert.reverts(this.streamPayOfInstallment.withdrawInstallmentStream(streamId, FIVE_UNITS, opts), "stream does not exist");
        });
    });
}
...
function runTests(feesOfProtocolPer) {
    describe("when not paused", function() {
        describe("when the withdrawal amount is higher than 0", function() {
            describe("when the stream did not start", function() {
                ...
                it("reverts", async function() {
                    await truffleAssert.reverts(
                        this.streamPayOfInstallment.withdrawInstallmentStream(this.streamId, withdrawalAmount, this.opts),
            ...
            streamStartOfOne(function() {
                describe("when the withdrawal amount does not exceeds the available balance",function() {
                    ...
                    it("withdraws from the stream", async function() {
                        ...
                        await this.streamPayOfInstallment.withdrawInstallmentStream(this.streamId, withdrawalAmount, this.opts);
                    ...
                    it("emits a withdrawfromstream event", async function() {
                        const result = await this.streamPayOfInstallment.withdrawInstallmentStream(this.streamId, withdrawalAmount, this.opts);
                        truffleAssert.eventEmitted(result, "WithdrawFromInstallmentStream");
                    });

                    it("decreases the stream balance", async function() {
                        const balance = await this.streamPayOfInstallment.installmentBalanceOf.call(this.streamId, this.recipient, this.opts);
                        await this.streamPayOfInstallment.withdrawInstallmentStream(this.streamId, withdrawalAmount, this.opts);
                        const newBalance = await this.streamPayOfInstallment.installmentBalanceOf.call(this.streamId, this.recipient, this.opts);
                    ...
                    it("getearnings",async function() {
                        const balance = await this.streamPayOfInstallment.getEarnings.call(this.token.address,this.opts);
                        await this.streamPayOfInstallment.withdrawInstallmentStream(this.streamId, withdrawalAmount, this.opts);
                        const newBalance = await this.streamPayOfInstallment.getEarnings.call(this.token.address,this.opts);
            ...
            describe("number of two",async function() {
                const now = new BigNumber(dayjs().unix());
                const withdrawalAmount = FIVE_UNITS.multipliedBy(241).toString(10);
            ...
            describe("number of three",async function() {
                const now = new BigNumber(dayjs().unix());
                const withdrawalAmount = FIVE_UNITS.multipliedBy(481).toString(10);
    ...
    describe("when paused", function() {
        ...
        beforeEach(async function() {
            ...
            await this.streamPayOfInstallment.pause({ from: this.sender });
        });
...
module.exports = shouldBehaveLikeInstallmentWithdrawStream;