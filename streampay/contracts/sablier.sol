...
/**
 * @title 流付/StreamPay's money streaming of fixedDepositStream
 * @author StreamPay
 * Thank you @PaulBerg,@Sablier.Their ideas give me a lot help.
 * FixedDepositStream content used for sablier,there just add the handleRemainder methods.
 */
 
/**
 * @title Sablier's Money Streaming
 * @author Sablier
 *         From https://github.com/sablierhq/sablier/blob/develop/packages/protocol/contracts/Sablier.sol
 */
contract Sablier is IERC1620,OwnableWithoutRenounce, PausableWithoutRenounce, Exponential, ReentrancyGuard{
    ...
    function balanceOf(uint256 streamId, address who) public view streamExists(streamId) returns (uint256 balance) {
        ...
        if(block.timestamp >= stream.stopTime){
            ...
            (vars.mathErr,vars.remainder) = modUInt(stream.deposit,stream.duration);
            assert(vars.mathErr == MathError.NO_ERROR);

            (vars.mathErr, vars.recipientBalance) = addUInt(vars.remainder, vars.recipientBalance);
            require(vars.mathErr == MathError.NO_ERROR, "recipient balance calculation error");
        }
     ...
     * @dev Throws if the id does not point to a valid stream.
     *  Throws if caller is not recipient.
